    ...
    try {
      ...
      if (res.data.code === 0 && res.data.data) {
        ...
        if (taskResult.taskStatus === 'SUCCEEDED') {
         ...
         if(taskResult.message==='InvalidParameter.ImageResolution'){
          message.error('扩图任务执行失败。图片尺寸不符合，请重新上传图片')
          // 清理轮询
          clearPolling() 
        }else if(taskResult.code==='InvalidParameter.ImageResolution'){
           message.error('扩图任务执行失败。分辨率不符合，请重新上传图片')
          // 清理轮询
          clearPolling()
        }
        }
